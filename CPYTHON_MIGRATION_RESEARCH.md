...
**The fix and its two venues.** Per an external source-level review (corroborated here except where noted): upstream pythonnet PR **#2055** — authored by pyRevit's own maintainer — adds exactly the missing machinery (generated-type caching, deterministic naming, no re-definition on reload) but was **closed unmerged upstream**, and its changes are absent from the pinned fork commit (absence confirmed from source; the PR's own metadata not independently re-verified — outside this session's GitHub scope). Because pyRevit ships its own fork, upstream's closure is not binding:
...
**Sequencing consequence (binding on Phase 1):** do **not** blanket-apply `__namespace__` to the 33 missing sites before the reload fix lands — deterministic names without a type cache make reload collisions *more* certain, not less. Question (a) — which interfaces need `__namespace__` at all — remains empirical: "need" is a property of whether the .NET side name-resolves or reflects on the generated type, not of the interface itself; settle it with a per-interface matrix (~6–8 distinct interfaces, tested with/without `__namespace__` × across a reload) via the DevTools button + parity suite. This finding also strengthens the §9.2 Tier 5 decision: with the `ExternalEvent` handler implemented once in C#, per-dialog Python never implements a .NET interface at all and sidesteps this entire class for the most common modeless case.
