...
- **Modeless / host-plumbing tier** (`ProgressBar`, dockable panels): the **C# window-host base is unconditional** — `ExternalEvent` marshaling, window ownership/parenting, theming (the `ScriptOutput` pattern, §9.2) ship in C# regardless of the binding-prototype outcome. Only the *binding backend* is gated; a spike pass does not mean "no C# anywhere."
