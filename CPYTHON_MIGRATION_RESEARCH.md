...
- **Autodesk shipped the engine fix — production precedent for venue 1.** The "PythonNet3" post: the .NET-inheritance bug "is fixed starting in PythonNet3 version 1.1.0. Inheritance now works just as you'd expect… *(Thanks to [pythonnet/pythonnet#2055] for the method.)*" — note "1.1.0" is the version of Dynamo's engine *package* named PythonNet3, not a pythonnet-library version (the library is pythonnet 3.x in both Dynamo's package and pyRevit's fork). Dynamo applied the closed PR's method in their own engine rather than waiting for upstream, independently confirming #2055's existence and content. After the fix, their example needs nothing but a plain `__namespace__` — no guard.
